...
/**
 * Create a new MCQ test session
 */
export function createMCQSession(config) {
  const { 
    topic = null,
    difficulty = null,
    questionCount = 10,
  ...
  const questions = getRandomQuestions(questionCount, topic, difficulty);
...
/**
 * Submit answer for MCQ question
 */
export function submitMCQAnswer(sessionId, questionIndex, userAnswer, timeSpent) {
  const session = getMCQSession(sessionId);
  if (!session) throw new Error('Session not found');
  ...
  saveMCQSession(session);
  return { isCorrect: question.isCorrect, explanation: question.explanation };
}

/**
 * Complete MCQ session and calculate score
 */
export function completeMCQSession(sessionId) {
  const session = getMCQSession(sessionId);
  if (!session) throw new Error('Session not found');
...
/**
 * Get MCQ session by ID
 */
export function getMCQSession(sessionId) {
  const sessions = getAllMCQSessions();
  return sessions.find(s => s.id === sessionId);
}

/**
 * Get MCQ history for current user
 */
export function getMCQHistory() {
  ...
  const stats = {
    ...
    averageScore: completedSessions.length > 0 
      ...
      : 0,
    highestScore: completedSessions.length > 0 
      ? Math.max(...completedSessions.map(s => s.score))
      : 0,
    totalQuestions: completedSessions.reduce((sum, s) => sum + s.totalQuestions, 0),
    correctAnswers: completedSessions.reduce((sum, s) => sum + s.correctAnswers, 0)
  };
...
/**
 * Get available MCQ topics and stats
 */
export function getMCQTopicsAndStats() {
  return {
    topics: getAllTopics(),
...
function getAllMCQSessions() {
  try {
    return JSON.parse(localStorage.getItem(MCQ_STORAGE_KEY)) || [];
  } catch {
    return [];
  }
}
...
function saveMCQSession(session) {
  ...
  localStorage.setItem(MCQ_STORAGE_KEY, JSON.stringify(sessions));
}

export default {
  // Original interview functions
